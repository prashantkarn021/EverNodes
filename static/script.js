...
function abbreviateLabel(label, scale) {
  ...
  if (scale >= 0.35) {
    // first ~12 chars of words
    const words = t.split(/\s+/);
    let out = "";
    for (const w of words) {
      const next = out ? out + " " + w : w;
      if (next.length <= 12) out = next;
      else break;
    }
...
function refreshAllLabels() {
    ...
    network.body.data.nodes.update(
        currentData.nodes.map(n => {
            ...
            // Only scale font — never touch widthConstraint
            // Scaling width causes text to escape the box boundary
            ...
            return {
                id:    n.id,
                label: abbreviateLabel(n.label, currentScale),
  ...
  function draw(ts) {
    ...
    stars.forEach((s) => {
      ...
      if (s.bright && a > 0.3) {
        ...
        ctx.fillStyle = glow;
        ctx.beginPath();
        ctx.arc(cx, cy, s.baseR * 7, 0, Math.PI * 2);
        ctx.fill();
      }
...
function applyPositions(posMap) {
  ...
  const updates = currentData.nodes
    .map((n) => {
      const p = posMap[n.id];
...
function renderSavedList(query = "") {
  ...
  if (!Object.keys(maps).length) {
    list.innerHTML =
      '<p class="empty-state">No saved maps yet.<br>Generate your first one!</p>';
    return;
  }

  if (!keys.length) {
    list.innerHTML = `<p class="empty-state">No maps match<br><em style="color:var(--gold)">"${query}"</em></p>`;
    return;
  }
  ...
  list.innerHTML = keys
    .map((key) => {
      const m = maps[key];
      const learned = (m.learned || []).length;
      const total = (m.data?.nodes || []).length;
      const date = new Date(m.savedAt).toLocaleDateString(undefined, {
        month: "short",
        day: "numeric",
      });

      // Highlight matching characters in the goal name
      const displayName = q
        ? m.goal.replace(new RegExp(`(${q})`, "gi"), "<mark>$1</mark>")
        : m.goal;

      return `<div class="saved-item" data-key="${key}">
            <div class="lvl-pip ${m.level || "beginner"}"></div>
            <div class="saved-item-info">
                <div class="saved-item-name">${displayName}</div>
...
function setGraphTitle(goalText) {
  ...
  btn.style.cssText =
    "background:none;border:none;color:var(--gold);font-size:0.72rem;cursor:pointer;font-family:var(--font-mono);margin-left:6px;padding:0;";
  btn.onclick = () => {
    expanded = !expanded;
...
function getClientCache() {
  try {
    return JSON.parse(sessionStorage.getItem(CLIENT_CACHE_KEY) || "{}");
  } catch {
    return {};
  }
}
...
async function generate() {
  ...
  if (!goal) {
    setStatus("Enter a topic to map.", true);
    goalEl.focus();
    return;
  }
  ...
  try {
    ...
    if (data.error === "invalid_topic") {
      setStatus(
        data.message || "That does not look like a learnable topic. Try again.",
        true,
      );
      setLoading(false);
      return;
...
function renderGraph(data, goal, savedPositions) {
  ...
  const visNodes = new vis.DataSet(
    data.nodes.map((n) => {
      ...
      vn.level =
        n.tier ??
        (n.category === "root" ? 0 : n.category === "concept" ? 1 : 2);
...
// STRAIGHT arrows — no smooth curve = zero line-behind artifact
...
function openPanel(nodeId) {
  ...
  cat.className =
    "cat-badge " + (isLearned ? "learned" : node.category || "concept");
  ...
  const prereqs = currentData.edges
    .filter((e) => e.to === nodeId)
    .map((e) => e.from);
  const nexts = currentData.edges
    .filter((e) => e.from === nodeId)
    .map((e) => e.to);
  wrap.innerHTML = [
    ...prereqs.map((id) => {
      const n = currentData.nodes.find((n) => n.id === id);
      return n ? `<span class="chip prereq">← ${n.label}</span>` : "";
    }),
    ...nexts.map((id) => {
      const n = currentData.nodes.find((n) => n.id === id);
      return n ? `<span class="chip next">→ ${n.label}</span>` : "";
    }),
  ].join("");
}

function closePanel() {
  document.getElementById("node-panel").classList.remove("panel-open");
}
...
function openPortal(nodeId) {
  ...
  if (explorerIndex < 0) {
    const node = currentData.nodes.find((n) => n.id === nodeId);
    if (node) {
      explorerList = [node, ...explorerList];
      explorerIndex = 0;
    } else return;
  }
  renderPortal();
...
function renderPortal() {
  ...
  document.getElementById("portal-topic-crumb").textContent = currentGoal
    .split(/\s+/)
    .slice(0, 6)
    .join(" ");
  document.getElementById("portal-node-crumb").textContent = node.label;
  document.getElementById("portal-nav-counter").textContent =
    `${explorerIndex + 1} / ${explorerList.length}`;
  document.getElementById("portal-prev-btn").disabled = explorerIndex <= 0;
  document.getElementById("portal-next-btn").disabled =
    explorerIndex >= explorerList.length - 1;
  ...
  document.getElementById("portal-parent-detail").innerHTML = parseDetail(
  ...
  // Children cards
  const childIds = currentData.edges
  ...
  if (!children.length) {
    grid.innerHTML =
      '<p style="color:var(--text3);font-size:0.85rem;grid-column:1/-1;">No deeper concepts from this node.</p>';
  } else {
    grid.innerHTML = children
      .map((child) => {
        const childCfg =
          NODE_CFG[child.category || "detail"] || NODE_CFG.detail;
...
document.getElementById("portal-prev-btn").onclick = () => {
  if (explorerIndex > 0) {
    explorerIndex--;
    renderPortal();
  }
};
document.getElementById("portal-next-btn").onclick = () => {
  if (explorerIndex < explorerList.length - 1) {
    explorerIndex++;
    renderPortal();
  }
};
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      if (p.shape === "star") {
        ...
        for (let j = 0; j < 4; j++) {
          ...
          ctx.moveTo(p.x, p.y);
          ctx.lineTo(
            p.x + Math.cos(ang) * s * 1.8,
            p.y + Math.sin(ang) * s * 1.8,
          );
        }
...
function toggleLearned(nodeId) {
  ...
  if (network && currentData) {
    ...
    if (node) {
      ...
      vn.level =
        node.tier ??
        (node.category === "root" ? 0 : node.category === "concept" ? 1 : 2);
...
document
  ...
  .addEventListener("click", () => saveCurrentMap(false));
...
(function() {
    ...
    // Also close on any click on the main dashboard area directly
  ...
  function drawShooters() {
    ...
    for (let i = shooters.length - 1; i >= 0; i--) {
      ...
      const grad = ctx.createLinearGradient(
        s.x,
        s.y,
        s.x - (s.vx * s.len) / s.spd,
        s.y - (s.vy * s.len) / s.spd,
      );
...
// Search inside saved maps sidebar
document.getElementById("sidebar-search").addEventListener("input", (e) => {
  renderSavedList(e.target.value);
});

// Clear search when sidebar closes so it resets for next open
(function () {
  ...
  observer.observe(document.getElementById("saved-sidebar"), {
    attributes: true,
